...
switch (process.env.NETWORK.toLowerCase()) {
    case "bsc":
        ...
        break;
    case "homestead":
        provider = new ethers.providers.AlchemyProvider(process.env.NETWORK, process.env.ALCHEMY_KEY);
        break;
    case "rinkeby":
        provider = new ethers.providers.AlchemyProvider(process.env.NETWORK, process.env.ALCHEMY_KEY);
        break
...
const ranges = [...Array(chunkCount).keys()].map(
    (i) => [startBlockNumber + i * BLOCK_LIMIT_PER_CALL, startBlockNumber + (i + 1) * BLOCK_LIMIT_PER_CALL - 1]
);
...
for (const range of ranges) {
    ...
    const tokenReceivers = new Set(tokenTransfers.map((t) => t.args.to));
    index = new Set([...tokenReceivers, ...index]);
    console.log(`${(100 - (endBlockNumber - range[1]) / blockCount * 100).toFixed(2)}%`);
}
...
for (const address of Array.from(index)) {
    ...
    console.log(`${(Object.keys(balances).length / index.size * 100).toFixed(2)}%`)
}
